...
export default {
  Query: {
    getTeamMembers: requiresAuth.createResolver(async (parent, { teamId }, { models, user }) => models.sequelize.query(
      `
  ...
  Team: {
    ...
    // distinct => we don't multipe users with same id
      models.sequelize.query('select distinct on (u.id) u.id, u.username from users as u join direct_messages as dm on (u.id=dm.receiver_id) where (:currentUserId=dm.sender_id or :currentUserId=dm.sender_id) and dm.team_id=:teamId', {
